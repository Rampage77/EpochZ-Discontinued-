...
SecPopulate = {
	...
	if (Debug) then {
		diag_log format["EpochZ: Spawning 1 Secondary Zombie	|	Position : %1	|	Class : %2 ",_position,_zClass];
	};
	_zClass createUnit
	[
		_position,
...
if !(_set) then
{
	if (UseBuildings) then {
		...
		if (Debug) then {
			diag_log format["EpochZ: Setting Secondary Trigger	|	Position : %1	|	Spawn Radius : %2m	|	GroupSize : %3	|	Buildings : %4	|	Spawn Positions : %5	|	Near : %6 ",_triggerPosition,SecSpawnRadius,_groupSize,Count _buildings,_posCount,_nearestLocation];
		};
	}
	else
	{	
		...
		_this select 0 setvariable ["groupSize",_groupSize, False];
		if (Debug) then {
			diag_log format["EpochZ: Setting Secondary Trigger	|	Position : %1	|	Spawn Radius : %2m	|	GroupSize : %3	|	Near : %4 ",_triggerPosition,SecSpawnRadius,_groupSize,_nearestLocation];
		};
	};
};

if (Debug) then {
	diag_log format["EpochZ: Activating Secondary Trigger	|	Position : %1	|	Spawn Radius : %2m	|	GroupSize : %3	|	Near : %4 ",_triggerPosition,SecSpawnRadius,_groupSize,_nearestLocation];
};
...
if !(triggeractivated (_this select 0)) then {
	...
	if (Debug) then {
		diag_log format["EpochZ: Deactivating Secondary Trigger	|	Position : %1	|	Spawn Radius : %2m	|	GroupSize : %3	|	%4 Zombie Deleted	|	Near :%5 ",_triggerPosition,SpawnRadius,_groupSize,_del,_nearestLocation];
	};
};
