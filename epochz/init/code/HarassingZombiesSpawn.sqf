...
HZPopulate = {
	...
	{
		if (Debug) then {
			diag_log format["EpochZ: No suitable spawn location found for a Harassing Zombie near %1 ",name (_this select 0)];
		};
	}
	else
	{	
		...
		if (Debug) then {
			diag_log format["EpochZ: Spawning 1 Harassing Zombie	|	Position : %1	|	Class : %2	|	For Player : %3 ",_zombieSpawnPosition,_zClass,name (_this select 0)];
		};
		_zClass createUnit
		[
			_zombieSpawnPosition,
...
//cleanup, delete zombie if it is too far from the player that spawned it.
{
	if ((_x distance (_this select 0)) > HZMaxDistance) then {
		...
		deleteVehicle _x;
		if (Debug) then {
			diag_log format["EpochZ: Deleting 1 Harassing Zombie	|	Position : %1	|	Class : %2	|	From Player : %3 ",Position _x,typeOf _x,name (_this select 0)];
		};
	};
