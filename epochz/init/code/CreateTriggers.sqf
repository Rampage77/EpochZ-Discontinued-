...
if (PregeneratePos && UseBuildings) then
{
	...
	if (Debug) then {
		diag_log format["EpochZ: Creating Primary Trigger		|	Position : %1 	|	Radius : %2m	|	GroupSize : %3	|	Buildings : %4	|	Spawn Positions : %5	|	Near : %6 ",_position,TriggerRadius,_groupSize,Count _buildings,_posCount,_nearestLocation];
	};
}
else
{	
	if (Debug) then {
		diag_log format["EpochZ: Creating Primary Trigger		|	Position : %1 	|	Radius : %2m	|	Near : %3 ",_position,TriggerRadius,_nearestLocation];
	};
};