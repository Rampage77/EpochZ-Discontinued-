...
if (isnil "_position") exitwith { Diag_log "EpochZ: Empty position";};
if (count _position < 1) exitwith { Diag_log "EpochZ: Require a position ARRAY";};
if (surfaceiswater _position) exitwith { Diag_log "EpochZ: Position is in the water";};
...
if (PregeneratePos && SecUseBuildings) then
{
	...
	if (Debug) then {
		diag_log format["EpochZ: Creating Secondary Trigger	|	Position : %1	|	Radius : %2m	|	GroupSize : %3	|	Buildings : %4	|	Spawn Positions : %5	|	Near : %6 ",_position,SecTriggerRadius,_groupSize,Count _buildings,_posCount,_nearestLocation];
	};
}
else
{	
	if (Debug) then {
		diag_log format["EpochZ: Creating Secondary Trigger	|	Position : %1	|	Radius : %2m	|	Near : %3 ",_position,SecTriggerRadius,_nearestLocation];
	};
};